...
var VCO = function(samplerate) {
  ...
  this.stream = new Float32Array(stream_length)
};
    ...
    switch (this.wave) {
      case WAVE.TRI:
        for (i = 0, imax = stream_length; i < imax; i++)
          this.stream[i] = this.TriNext(this.glide.next(p)) * this.gain;
        break;
      case WAVE.SAW:
        for (i = 0, imax = stream_length; i < imax; i++)
          this.stream[i] = this.SawNext(this.glide.next(p)) * this.gain;
        break;
      case WAVE.SQUARE:
        for (i = 0, imax = stream_length; i < imax; i++)
          this.stream[i] = this.SquareNext(this.glide.next(p)) * this.gain;
        break;
    }
  } else {
    for (i = 0, imax = stream_length; i < imax; i++) {
